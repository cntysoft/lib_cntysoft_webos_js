...
Ext.define('WebOs.Kernel.ProcessModel.Runable', {
   ...
   constructor: function(config)
   {
      ...
      this.serviceTasks = new Ext.util.HashMap({
         keyFn: function(serviceTask){
    ...
    * @param {Object} config
    * @returns {WebOs.Kernel.ProcessModel.AbstractServiceTask}
    */
   getServiceTask: function(key, shared, config)
   {
      shared = Ext.isEmpty(shared)?!!shared:false;
      if(shared&&this.serviceTasks.containsKey(key)){
         return this.serviceTasks.get(key);
      }
      //创建新的widget对象
      if(!Ext.isEmpty(this.serviceTaskMap[key])){
         var cls = this.serviceTaskMap[key];
         config = Ext.isObject(config)?config:{};
         Ext.apply(config, {
            serviceInvoker: WebOs.ME.G_OBJ_REFS.SERVICE_INVOKER,
            runnable: this
         });
         var serviceTask = Ext.create(cls, config);
         if(!(serviceTask instanceof WebOs.Kernel.ProcessModel.AbstractServiceTask)){
            Cntysoft.raiseError(
                    Ext.getClassName(this),
                    "getServiceTask",
                    "serviceTask must instanceof WebOs.Kernel.ProcessModel.AbstractServiceTask"
                    );
         }
         if(shared){
